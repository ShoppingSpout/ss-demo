...
$qr=mysql_query("SELECT D.*, DATE_FORMAT(deal_end_date,'%M %d, %Y') as deal_end_date, S.store_name,c_store_logo FROM deals_coupons D, product_stores S WHERE D.sourceID = S.affliate_store_id AND dealFor = 'store_page'  AND S.store_name = 'hot new deals' AND (deal_end_date >= now() OR  deal_end_date = '0000-00-00 00:00:00') AND approved = 1 order by position") or die(mysql_error());
