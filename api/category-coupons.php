...
$res = mysql_query('SELECT * FROM category_coupons WHERE category_id = '.$cat_id) or die("Category not found. Please enter correct category_id");

	$i = 0;
	
	if (mysql_num_rows($res))
	{
		while($deal_row = mysql_fetch_object($res))
		{
			$couponIDs .= $deal_row->coupon_id.',';
		}
	}
	
	$couponIDs = rtrim($couponIDs,",");
	
if($couponIDs)
{

$qr=mysql_query("SELECT D.*, DATE_FORMAT(deal_end_date,'%M %d, %Y') as deal_end_date, S.store_name,c_store_logo FROM deals_coupons D, product_stores S WHERE D.sourceID = S.affliate_store_id AND dealFor = 'store_page'  AND dcID IN (".$couponIDs.") order by position") or die(mysql_error());
