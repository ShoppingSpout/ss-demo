...
$qr=mysql_query("SELECT D.*, DATE_FORMAT(deal_end_date,'%M %d, %Y') as deal_end_date, S.store_name, c_store_logo, S.affliate_store_id, S.store_website FROM deals_coupons D, product_stores S WHERE D.sourceID = S.affliate_store_id AND dealFor = 'store_page'  AND (deal_end_date >= now() OR  deal_end_date = '0000-00-00 00:00:00') AND approved = 1 AND D.dcID = ".$coupon_id." order by position") or die("Coupon not found. Please enter correct coupon_id.");
...
while($res=mysql_fetch_array($qr))
{
	...
	$more_offer_url = 'http://www.shoppingspout.com.au/'. str_replace(" ","-", $res['store_name']).'-coupon-codes.html';
	$link['more_offers_url']= $more_offer_url;
	$link['store_id']= $res['affliate_store_id'];
	$link['store_website']= $res['store_website'];
